...
{
    /*
     * oracle 没有数据库的概念，但有表空间，差不多的意思
     * 由cmd进入 sqlplus
     * sqlplus / as sysdba
     * 
     * 1、创建表空间
     * create tablespace singoocms datafile 'F:\jsonlee\oracle\db\singoocms.dbf' size 10m;
     * 
     * 2、创建用户
     * create user jsonlee identified by 123 default tablespace singoocms;
     * 
     * 3、授予权限
     * grant dba,connect to jsonlee;
     * 
     */
    ...
    public class OracleAdoTest
    {
        ...
        public async Task InsertTest()
        {
            ...
            if (dbMaintenance.ExistsTable("DbMaintenanceTest"))
            {
                ...
                {
                    await dbOracleAccess.InsertModelAsync(new DbMaintenanceTestInfo() { AutoID = i + 1, UserName = "jsonlee" });
                }
            }
            watch.Stop();
